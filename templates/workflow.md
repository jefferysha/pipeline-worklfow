...
- **Shell-driven 状态变更（硬规则）**：`.pipeline.yaml` 一律经 pipeline CLI 读写，**禁止直接 Edit**：
  ...
  - `pipeline check <name>` 过成功出口 guard（只校验不推进）；review 相位通过后还须
    `pipeline review request <name> --event <event>` → 人类确认 → `pipeline review acknowledge <name>` →
    `pipeline transition <name> <event>` 推进。多出口 review step（default 的 verify / 自定义 workflow）必须指定 event。
...
- 进入 review 相位（explore/spec/verify）后可以正常完成本相位工作；**不得**因为刚进入就写
  review marker。完成产物并选择待走的出边后，运行 `pipeline review request <name> --event <event>`：
...
- 先把产物路径、摘要和待确认取舍展示给用户。用户明确确认后，Codex 档 A/B 的 UserPromptSubmit hook
  调用 `pipeline review acknowledge <name>` 写入 approval receipt 并清投影；支持 AskUserQuestion 的宿主
  仅在回答中明确批准时做同一动作。档 C 必须保留确认事实并显式运行 acknowledge，**不得删 marker 绕过**。
- `pipeline transition` 只接受当前 review phase 的 exact approved receipt（同一个 phase 的不同 event 不可互用；
...
5. **明确持续授权是 Change 绑定的委托，不是总开关**：用户明确说“后续不用问 / 自主执行完成”时，入口以
   `pipeline session activate <change> --continuous` 写入只绑定该 live Change 的版本化授权投影。
   交互式 skill 可对低风险细节采用保守默认，并把假设/理由写入产物；它不得跨 Change 继承，用户说
   “恢复逐步确认 / 撤回自主执行”即可撤回。该投影允许在 **真实 review 证据、OpenSpec 文档读取收据与
   guard 均已通过之后**，用 `pipeline review acknowledge <change> --delegated` 记录一条带授权时间的
