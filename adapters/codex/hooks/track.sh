#!/usr/bin/env bash
# adapters/codex/hooks/track.sh — Codex PostToolUse track 薄包装（档 A 全保真）。
#
# contract §1：工具调用完成后把记录 append 到 openspec/changes/<name>/.pipeline-history.jsonl。
# 本 wrapper 复用 lite baseline hooks/skill-tracker.sh（它读 stdin JSON，仅追踪 Skill/Agent/Task，
# 提取名字 → append {"ts","kind":"tool","raw":"<Tool>: <name>"} 到活跃 change 的 history）。
#
# 只做：argv($2)+stdin 双吃 → 归一 stdin JSON → 设 CLAUDE_PLUGIN_ROOT → 透传。
# track 是 fail-safe：失败不回滚、永远 exit 0（contract §1）。
# 用法（Codex hooks.json）：command = "<adapter>/hooks/track.sh PostToolUse"
set -uo pipefail
