#!/usr/bin/env bash
# adapters/continue/hooks/track.sh — Continue CLI（`cn`）PostToolUse track 薄包装（档 A 全保真）。
#
# contract §1：工具调用完成后把记录 append 到 openspec/changes/<name>/.pipeline-history.jsonl。
# 本 wrapper 复用 lite baseline hooks/skill-tracker.sh（同 codex/gemini/cursor/copilot/pi 做法）。
#
# 只做：argv($2)+stdin 双吃 → 归一 stdin JSON → 设 CLAUDE_PLUGIN_ROOT → 透传。
# track 是 fail-safe：失败不回滚、永远 exit 0（contract §1）。
# 用法（Continue CLI hooks）：command = "<adapter>/hooks/track.sh PostToolUse"
set -uo pipefail
