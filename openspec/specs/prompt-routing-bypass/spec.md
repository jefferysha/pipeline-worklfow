# prompt-routing-bypass Specification

## Purpose

允许维护者为单个项目配置一个只作用于当前提示轮次的路由旁路词，在不关闭 review、confirm、
PreToolUse 或安全门的前提下，抑制 Tenon router 与 breadcrumb 输出；同时由 Dashboard、API 和
canonical Hook 配置提供一致、可恢复且向后兼容的管理闭环。
