# codex-skill-receipt-current-turn Specification

## Purpose

确保 Codex 在 sibling worktree 中读取 Tenon Skill 时，只有同一宿主轮次、完整成功结果和可信项目身份均可证明的调用才能生成文档证据，从而让无人值守调度在首轮推进并保持失败关闭。
