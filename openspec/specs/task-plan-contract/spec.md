# task-plan-contract Specification

## Purpose

定义版本化 TaskPlan/WorkItem 的稳定身份、显式覆盖与依赖、资源冲突、原子 revision store、
legacy `tasks.md` 兼容边界及稳定只读 API，使计划、验证和后续调度可绑定可审计的 canonical revision。
