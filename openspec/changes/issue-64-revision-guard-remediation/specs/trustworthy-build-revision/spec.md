...
CLI check/review/transition、transition HTTP、snapshot/SSE、AFK settlement 与 Dashboard MUST 使用 code
...
#### Scenario: Interactive forward admission is rejected

- **WHEN** CLI check、exact-event review request、CLI transition 或 server transition 遇到不可信 revision
- **THEN** 所有入口输出相同 code、reason、remediation 与 hash 语义
- **AND** HTTP 使用非成功状态，CLI 使用非零退出，canonical state 与 review receipt 均不变。
...
- **THEN** actual transition、check、review preflight 与 readiness 都返回同一 hard blocker
- **AND** 不得生成 `ready=true`、成功 receipt 或成功 transition。
...
- **THEN** revision guard 不改变既有 authorized 路径
- **AND** 重试同一事实得到相同判定。
...
- **THEN** runtime 不自动制造不存在的 Verify phase
