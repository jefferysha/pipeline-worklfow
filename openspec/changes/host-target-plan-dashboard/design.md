...
- Codex 的手工 setup/update 在 managed runtime 后包含 `codex-auth-status`；setup 随后才进入 skills/readiness。该步骤只描述真实的 `codex login status` 观察与引导，计划生成本身仍不探测登录状态。Claude 不包含该步骤。
