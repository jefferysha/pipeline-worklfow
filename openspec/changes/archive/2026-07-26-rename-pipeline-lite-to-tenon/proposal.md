...
- 当前检出树不保留外部参考项目的身份名称；相关调研、演示和归档产物从当前树删除，恢复边界仅为
  Git 提交历史，不改写既有提交对象。
...
- 增加仓库级参考身份门禁，同时检查受 Git 管理的路径和文本；任一命中都使 CI、打包和发布失败。
...
Explore 阶段将枚举全部现行引用、Git 对象恢复边界、执行来源数据流、仓库资产分类、一步安装契约以及
仓库自举和发布顺序。Spec 返工将把“归档允许保留参考身份”修订为“当前树零明文”。
