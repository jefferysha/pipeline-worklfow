...
- 在项目 Dashboard 快照中增加 optional、只读、无路径泄露的 `unsupported-canonical-version` issue；
  最多返回 100 条，并用 optional 字面量 `compatibilityIssuesTruncated: true` 表达其余项被省略，
  不把溢出降级为普通项目错误。
- 提供中英文升级说明、升级后刷新，以及对 snapshot/网络失败按当前语言展示的通用重试入口。
