...
- [x] 以物理 cwd containment 排除 symlink 后代逃逸，并对无法解析的历史 cwd 返回 honest partial。 (build)
- [x] 以 sidecar-free immutable SQLite 读取避免 OpenCode WAL/SHM 宿主写入，live WAL 与并发漂移 fail closed。 (build)
