...
- snapshot 增加 optional 项目级 issue，只暴露 Change 名、发现版本、支持版本、稳定原因码与 `upgrade-runtime` action，不暴露本机绝对状态路径或原始错误；最多返回 100 条，溢出只由 optional 字面量 `compatibilityIssuesTruncated: true` 表达。
- Dashboard 在当前项目 Progress 入口显示升级要求，优先于 no-change 教学空态，并复用已有 refresh；snapshot/网络失败按当前语言显示并提供通用重试，本功能不自动执行更新。
...
- 共存模型：`ProjectSnapshot.ok=false`，`compatibilityIssues` 与其他可读 `changes` 并存；issue 数组按 Change 名稳定排序并截取前 100 条，余量用 typed truncation metadata 表达，不写入自由文本 `error`。
- 交互模型：Progress notice 调用 App 现有 `refresh`；loading 时禁用，缺失/空数组不渲染；全局请求错误按 locale 渲染，并在现有请求通道上提供通用 retry。
