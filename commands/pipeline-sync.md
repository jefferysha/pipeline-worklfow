...
**输入**（`pipeline sync` CLI，经单文件 bundle `packages/cli/dist/pipeline.mjs`）：
- `pipeline sync`（默认，**只报告不改盘**）
- `pipeline sync --migrate`（opt-in 执行路径迁移；breaking 升级必须显式带）
- `pipeline sync --allow-downgrade`（CLI 版本低于项目时强制降级）
- `pipeline sync banner`（项目版本戳 vs CLI 版落后 nudge，零网络）
- `pipeline sync upgrade-channel`（从 `installed_plugins.json` 派生 latest/beta/rc）
...
并把 `sync.ts` 的相对桥 import 改回 `@pipeline-lite/kernel`（待 barrel 导出 ownership）。
