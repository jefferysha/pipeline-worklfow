---
name: "Pipeline: Pass"
description: 放行当前复核门——把 change 按状态机向前推进一相（pipeline transition <change> <前进 event>）
category: Workflow
...
**向前推进一相**，等价于底层 `pipeline transition <change> <前进 event>`。
...
1. **定位 change**：若给了名字直接用；否则跑 `pipeline inbox --json` 看等待人工决策的 change
   （复核相位 = explore/spec/verify），从中挑当前这个；仍不确定就把候选列给用户拍板，别猜。
2. **定位前进 event**：读当前相位 `pipeline get <change> phase`，按下表取该相位的前进 event：
...
3. **执行放行**：`pipeline transition <change> <前进 event>`（成功 `[TRANSITION] name: old -> new` 走 stderr，exit 0）。
