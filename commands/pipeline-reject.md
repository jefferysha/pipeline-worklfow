...
**回退**，等价于底层 `pipeline transition <change> <回退 event>`。
...
1. **定位 change**：给了名字直接用；否则 `pipeline inbox --json` 找处于复核门的 change，不确定让用户拍板。
2. **定位回退 event**：读 `pipeline get <change> phase`。默认 workflow 的**唯一回退边**是：

   | 当前相位 | 回退 event | 落到 | 副作用 |
   |----------|-----------|------|--------|
   | verify | `verify-fail` | build | `verify_result=fail` + `build_sha=null`（下轮 build 重新冻结 SHA） |

3. **执行打回**：`pipeline transition <change> verify-fail`（exit 0，`[TRANSITION]` 走 stderr）。
...
- **非 default workflow**：回退边由自定义 workflow 的 step 出边声明，按 `pipeline transition` 的
