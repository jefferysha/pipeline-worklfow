...
**输入**（`pipeline uninstall` CLI，经单文件 bundle `packages/cli/dist/pipeline.mjs`）：
- `pipeline uninstall`（无 `--yes` → fail-closed 拒绝：脚本/非交互环境必须显式确认）
- `pipeline uninstall --dry-run`（仅预览删除计划，不动任何文件）
- `pipeline uninstall --yes`（或 `-y`，确认卸载；脚本/CI 必需）

## 不可阉割语义（#24 验收面；顺序铁律，删一条视为 critical 阉割）

0. **homedir 守卫**：`cwd == $HOME` 且无旁路 → HARD STOP exit 1（会牵连 `~` 的运行时数据）；
   `PIPELINE_ALLOW_HOMEDIR=1` 严格旁路。守卫先于任何写盘/删除。
...
并把 `uninstall.ts` 的相对桥 import 改回 `@pipeline-lite/kernel`（待 barrel 导出 ownership）。
