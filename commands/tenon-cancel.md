...
description: 终止运行中的 AFK 任务——落取消标记 + docker kill 沙箱容器（tenon afk cancel <change>）
category: Workflow
...
等价于底层 `tenon afk cancel <change>`（= dashboard 的 POST /api/afk/:name/cancel 终端等价）。
...
1. **定位 change**：给了名字直接用；否则 `tenon afk status` 看 `running` 泳道里的 change，不确定让用户拍板。
2. **执行终止**：`tenon afk cancel <change>`（可加 `--json`）。
