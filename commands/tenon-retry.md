---
name: "Pipeline: Retry"
description: 重试/重新入队 AFK 任务——tenon afk enqueue <change>（不是 run，run 忽略 name 跑整轮）
category: Workflow
...
等价于底层 `tenon afk enqueue <change>`。
...
1. **定位 change**：给了名字直接用；否则 `tenon afk status` 看 `failed`/`conflict`/`paused` 泳道里
   要重试的 change，不确定让用户拍板。
2. **重新入队**：`tenon afk enqueue <change>`（可加 `--json`）。**务必用 `enqueue`，不是 `run`**——
...
- **入队 ≠ 真跑**：`enqueue` 只把 change 挂回 `queued`。真正在沙箱里重跑要有 **docker daemon** 且再跑一轮
  `tenon afk run`（run 需 docker + 在 git 仓非 detached HEAD；无 docker → 诚实报告就绪队列、不伪装已执行，
