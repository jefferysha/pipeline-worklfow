...
1. Resolve an explicitly named change, or list active changes with `tenon list --json`. If the
   user did not identify one and more than one exists, ask them to choose.
2. Run `tenon status <change>` and read `proposal.md`, `design.md`, `tasks.md`, relevant ADRs,
...
5. Before leaving a governed phase, call `tenon document read <change> all` so later phases have
...
`openspec-propose` or `tenon-open` skill to create a new change.
