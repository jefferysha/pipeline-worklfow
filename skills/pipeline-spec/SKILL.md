---
name: pipeline-spec
description: "Pipeline Phase 3: Spec · 规格 + 实施计划。PM Track 画用户旅程图，frontend/backend Track 产出 Plan + delta spec。"
---
...
1. 使用 Skill 工具加载 `superpowers:brainstorming`（**必做，但按上面「硬姿态」运行**，别走过场）。
...
1. 使用 Skill 工具加载 `pipeline-lite:openspec-propose`。**禁止跳过此步骤**。
   - 用于：生成 delta spec
   - 产出：`openspec/changes/<name>/specs/<capability>/spec.md`

2. 使用 Skill 工具加载 `superpowers:writing-plans`。**禁止跳过此步骤**。
...
> 下面三条针对 `superpowers:writing-plans` 产出的 plan。写完 plan 按这三条自检，缺哪条补哪条。
...
```bash
pipeline set "$PIPELINE_CHANGE_NAME" plan "docs/superpowers/plans/$(date +%Y-%m-%d)-<feature>.md"
```
...
- `plan` 字段非空且文件存在（frontend/backend；pm 豁免）
...
- external-skill: superpowers:brainstorming · 强制（pm）
