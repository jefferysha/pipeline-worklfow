...
1. Read `pipeline status <change>` and `pipeline document status <change>`. Confirm that
...
3. Check `pipeline task children <change>` for active dependent changes and present any blocking
...
Use `pipeline document read <change> all` before the final archive check so the archive phase has a
