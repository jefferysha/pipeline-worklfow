...
# selected Change, so a normal-dialogue confirmation must call `pipeline review acknowledge`
...
PIPELINE_REVIEW_MARKER_PROTOCOL='pipeline-review-v2'
...
pipeline_review_marker_change() { # $1=marker path
  ...
  [ "$first" = "$PIPELINE_REVIEW_MARKER_PROTOCOL" ] || return 1
...
pipeline_review_marker_is_v2() { # $1=marker path
  ...
  [ "$first" = "$PIPELINE_REVIEW_MARKER_PROTOCOL" ]
}
...
# files itself; `pipeline review acknowledge` owns both the receipt and its projection.
pipeline_acknowledge_active_review() { # $1=verified project root $2=hook directory [$3=delegated]
  ...
  case "$mode" in
    delegated) ( cd "$root" && command pipeline review acknowledge "$active" --delegated ) >/dev/null 2>&1 ;;
    manual) ( cd "$root" && command pipeline review acknowledge "$active" ) >/dev/null 2>&1 ;;
    *) return 1 ;;
  esac
}
