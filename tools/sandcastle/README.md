...
`tenon afk run` 真跑一个 change 时的容器执行基座（BACKLOG #29-wire）。详见 `Dockerfile` 顶部注释。

## 本机构建

```sh
npm run build          # 先产出 packages/cli/dist/tenon.mjs（镜像 COPY 依赖它）
...
`tenon afk run`（不传 `--image`）默认用 `sandcastle:local`；真 agent 编码需要在环境里另外提供
...
发布后，`tenon afk run --image ghcr.io/<owner>/sandcastle:local` 即可跨机使用，无需本机重新构建。
