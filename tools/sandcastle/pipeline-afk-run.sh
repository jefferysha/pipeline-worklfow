...
#
# 两种模式（同一脚本、诚实分流）：
#   · 生产 agent 模式：按 PIPELINE_RUNNER 调真实 Codex/Claude CLI；缺 CLI/认证一律非零退出。
#   · 测试 fallback：仅 PIPELINE_TEST_ALLOW_DETERMINISTIC_FALLBACK=1 显式开启，产出单独
...
# PIPELINE_SKILL_BUNDLE_DIR 指向的容器私有固定目录，并以 root 去掉整棵树的写位。这里必须先直接
...
step_prompt_suffix=""
if [ -n "${PIPELINE_WORKFLOW_STEP_PROMPT_B64:-}" ]; then
  if ! decoded_step_prompt="$(node -e "const raw = process.env.PIPELINE_WORKFLOW_STEP_PROMPT_B64 || ''; process.stdout.write(Buffer.from(raw, 'base64url').toString('utf8'))")"; then
...
# 分派分支（下方 if/elif）都在本段之后，起 agent 前必须先过这一关。PIPELINE_SKILL_BUNDLE_DIR 缺席
...
# 同一算法的聚合 digest，与宿主在 docker run 时经环境变量单独注入的 PIPELINE_SKILL_BUNDLE_SHA256
...
# merge）对这条运行期路径自动生效。
if [ -n "${PIPELINE_SKILL_BUNDLE_DIR:-}" ]; then
  verify_script="/tmp/.pipeline-verify-skill-bundle.js"
  cat > "$verify_script" <<'PIPELINE_SKILL_BUNDLE_VERIFY_JS_EOF'
'use strict'
...
function main() {
  const bundleDir = process.env.PIPELINE_SKILL_BUNDLE_DIR || ''
  const expectedDigest = process.env.PIPELINE_SKILL_BUNDLE_SHA256 || ''
  if (!bundleDir) fail('PIPELINE_SKILL_BUNDLE_DIR 未设置（容器私有目录接线错误）')
  if (!expectedDigest) fail('PIPELINE_SKILL_BUNDLE_SHA256 未设置（冻结 digest 接线错误）')
  ...
  if (digest !== expectedDigest) {
    fail('重算聚合 digest（' + digest + '）与宿主注入的 PIPELINE_SKILL_BUNDLE_SHA256（' + expectedDigest + '）不一致——快照可能在传输期间被篡改')
  }
}
...
PIPELINE_SKILL_BUNDLE_VERIFY_JS_EOF
  ...
  if node "$verify_script"; then
    ...
    skill_bundle_prompt_suffix=" Before starting, read ${PIPELINE_SKILL_BUNDLE_DIR}/manifest.json and the SKILL.md file(s) it references under ${PIPELINE_SKILL_BUNDLE_DIR}/skills/ — this is read-only execution guidance frozen for this run; the directory will not change while you work."
    export skill_bundle_prompt_suffix
  else
    rm -f "$verify_script"
    exit 94
...
# (notably pipeline runtime metadata). After the agent exits, the parent shell computes the complete Git
...
reconcile_agent_workspace() {
  ...
  gate_exit=0
  pipeline internal-constraint-gate write "$paths_file" || gate_exit=$?
...
if [ -n "${PIPELINE_AUTOMATION_POLICY_B64:-}" ]; then
  prepare_agent_workspace
fi
...
if [ "${PIPELINE_RUNNER:-}" = "codex" ]; then
  ...
  PIPELINE_TAP_DIR="$tap_dir" pipeline tap start codex --forward --ca /tmp/sandcastle-tap-ca -- \
    sh -c 'timeout 300 codex exec --json -C "$2" --sandbox workspace-write --ephemeral --ignore-user-config --ignore-rules "Implement the pipeline build task for change $1. First read AGENTS.md and openspec/changes/$1/REAL_AGENT_TASK.md.${step_prompt_suffix}${attempt_context_prompt_suffix}${skill_bundle_prompt_suffix} You must edit the business tree to complete that task; do not only describe or inspect. Do not run pipeline transitions and do not commit. Stop after the requested files are correct." >"$3/.sandcastle-build.agent.jsonl" 2>"$3/.sandcastle-build.agent.log"' _ "$name" "$agent_workspace" "$run_worktree" \
    || agent_exit=$?
  if [ "$agent_exit" -eq 0 ]; then
    if ! provider_usage_json="$(pipeline internal-codex-jsonl usage ".sandcastle-build.agent.jsonl")"; then
      printf 'codex JSONL 用量解析失败：拒绝把畸形 provider 事件当作成功执行\n' >&2
      agent_exit=98
    fi
    if ! pipeline internal-codex-jsonl transitions ".sandcastle-build.agent.jsonl"; then
  ...
  if [ -n "$tap_port" ]; then
    ( cd "$agent_workspace" && \
      ANTHROPIC_BASE_URL="http://127.0.0.1:${tap_port}" \
        timeout 300 claude -p "Implement the pipeline build task for change ${name}. First read AGENTS.md and openspec/changes/${name}/REAL_AGENT_TASK.md.${step_prompt_suffix}${attempt_context_prompt_suffix}${skill_bundle_prompt_suffix} You must edit the business tree to complete that task; do not only describe or inspect. Do not run pipeline transitions and do not commit. Stop after the requested files are correct." \
  ...
  if [ "${PIPELINE_TEST_ALLOW_DETERMINISTIC_FALLBACK:-}" != "1" ]; then
    printf '未检测到 CLAUDE_CODE_OAUTH_TOKEN 或可用 claude CLI；agent 未真跑，生产 fallback 默认关闭\n' >&2
    exit 98
  fi
  execution_mode="deterministic-test-fallback"
  printf 'TEST-ONLY deterministic fallback enabled by PIPELINE_TEST_ALLOW_DETERMINISTIC_FALLBACK=1; no real agent ran\n' >&2
fi
...
if [ "$execution_mode" != "deterministic-test-fallback" ]; then
  ...
  if [ "$agent_head" = "$initial_head" ] && [ -z "$meaningful_status" ]; then
    if [ "$execution_mode" = "agent/codex" ] \
      && codex_last_message="$(pipeline internal-codex-jsonl last-message ".sandcastle-build.agent.jsonl" 2>/dev/null)" \
