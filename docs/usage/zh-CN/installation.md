...
```bash
/usr/bin/curl -fsSL https://raw.githubusercontent.com/jefferysha/tenon/v1.0.7/install.sh | /bin/bash -s -- --codex
```

安装 Claude Code：

```bash
/usr/bin/curl -fsSL https://raw.githubusercontent.com/jefferysha/tenon/v1.0.7/install.sh | /bin/bash -s -- --claude
```
...
```bash
/usr/bin/curl -fsSL https://raw.githubusercontent.com/jefferysha/tenon/v1.0.7/install.sh | /bin/bash -s -- --codex --dry-run
```

版本化脚本只使用不可变稳定版本 `v1.0.7` 的预构建资产，不 clone 仓库、不执行源码编译。
