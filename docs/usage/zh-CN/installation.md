...
本指南完成 Pipeline Lite 的一次性安装、宿主选择、运行时检查和第一轮验证。安装时必须显式选择宿主，避免把 Codex、Claude Code 或其他平台的 hook/skill 目录混在一起。

## 目标

完成后，你会得到一个用户级、可更新、可回滚的 Pipeline Lite 安装；选定宿主能在新会话加载对应 adapter，`pipeline` CLI 能找到受管 runtime，Dashboard 能在本机 loopback 打开。
...
不要省略宿主参数。`pipeline setup` 需要知道要写入哪个宿主的 skills、hooks 和 manifest；这不是能从当前 shell 可靠猜出的信息。
...
- `pipeline doctor` 不会把缺失 runtime、损坏 skill 或失效 hook 报成绿色；
- `pipeline runtime status` 显示当前激活 release 和校验状态；
- `pipeline dashboard --open` 打开 Pipeline Lite，而不是其他占用端口的应用；
...
- skill 缺失：运行 `pipeline doctor`，再执行对应宿主的 repair/setup；
- hook 未生效：关闭旧会话并新建会话；
- Dashboard 打开了其他应用：核对页面标题和实际监听进程；
- setup 报已有不同宿主配置：使用明确的 `--codex` 或 `--claude`，不要手工混合目录；
- runtime 校验失败：先运行 `pipeline runtime status`，再按受控流程 repair/rollback；
