...
- **未来 canonical schema 兼容面**：kernel 解码器遇到 `current.json.schemaVersion` 高于本 runtime
  ...
  `action=upgrade-runtime`），对应 Change 不进入 `changes[]`，项目 `ok=false`；超过上限时截断并在
  `error` 中 fail-loud。普通 corruption 可与该数组同时存在，`error` 优先决定项目不可达，不能被升级
  提示掩盖。仅有未来版本问题时 Dashboard 可只读展示已成功解码的 sibling Changes 与升级提示，但
  唯一允许的动作是刷新 snapshot；创建、transition/cancel、AFK 和 Workbench 写入口全部继续要求
  `project.ok=true`。旧 server 省略 `compatibilityIssues` 时前端保持兼容。
