...
| 上游 B（受仓库文本卫生策略限制，完整链接在 PR 描述） | `main` / `c94d6fc289b7a6fdd9480bdfae4d4639c9ac2d4c` | 无 GitHub Release（latest endpoint 404）；稳定语义 tag `v0.6.10`，同 SHA。`v0.7.0-beta.1` 是 prerelease，不当作 stable。 | 2026-07-29 已读取 repository、release 与 tag 一手接口。 |
| 上游 C（受仓库文本卫生策略限制，完整链接在 PR 描述） | `master` / `5927d720c07aae80350511e1e2e611546c201ab4` | latest release 为 prerelease `0.4.0-beta.9`；稳定 tag 回退为 `0.3.9` / `053f76d8ac6aaa499b1d3f8752cb5637fc4fb914`。 | 2026-07-29 已读取 repository、release 与 tag 一手接口。 |
...
| 上游 B 的已固定任务产物设计（完整链接在 PR 描述） | Artifact consumers have a defined read order; optional artifacts are skipped; its SessionStart context stays compact and loads details on demand. It explicitly declines a new persistent artifact-metadata store. | Preserve one canonical Tenon ledger and expose only a compact per-document fact. Do not introduce a second “timeline database”, inline document bodies, or infer a read from artifact presence. |
| 上游 C 的已固定架构文档（完整链接在 PR 描述） | It separates user-facing state from resumable run state and stores trajectory/context/artifacts/checkpoints so interrupted work can be recovered from evidence rather than agent recollection. | A Dashboard event needs identity binding, not just a friendly timestamp: the read projection must remain tied to the currently effective digest and run visit. Tenon already owns those facts; no external runtime is needed. |
...
| --- | --- | --- |
| New persistent timeline/event store | Reject | Duplicates ledger/run history, adds mutation/concurrency/reconciliation risk, and contradicts upstream B's useful “no new artifact metadata store” boundary. |
