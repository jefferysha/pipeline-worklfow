...
- 任意非法 JSON、未知/多余键、版本或 kind 不同都返回不等价；嵌套 observation HEAD 虽不参与等价比较，也必须是 `null` 或 40 位小写 Git OID。
...
- 被忽略字段若不先做 canonical domain 校验会吞掉损坏证据；decoder 必须先拒绝非 canonical HEAD。
...
- 真实接线回归必须覆盖 native desired producer、command injection、durable journal reload 与通用 runner，防止 forwarding 被静默移除。
