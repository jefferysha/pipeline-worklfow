...
在本机执行构建。既有用户只运行 `tenon update --codex` 即可发现并切换到最新稳定 Release。
...
更新目标状态为 `unresolved -> resolved -> host-rebinding -> candidate-verified -> runtime-active ->
dashboard-ready -> evidence-committed`。`resolved` 前失败为纯只读失败；`host-rebinding` 之后失败保留 WAL，
...
| --- | --- | --- | --- |
| 交互式首次 `tenon setup` | 是，等待 readiness | 是；失败不回滚健康 runtime | 已验证 URL、失败时手动 URL |
...
asset 不一致、无 `main` 发布源、Dashboard 五类场景、open 失败降级和 host target plan。发布前还运行完整
