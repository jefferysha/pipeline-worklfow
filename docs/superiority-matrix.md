...
| D4 | 真实工具链验证 | check/guard 46 规则全量面（老 guard verdict 逐字一致）+ oracle 双跑 | trellis-check（lint/type/test） | **>**（校验面）· ~（三轨 verify 待 #29c） | guard.test + oracle 0 不一致；#12 |
...
| D7 | 多平台策略面 | 适配器框架 + registry 能力矩阵单源 + conformance 机器校验 + 分档降级 A/B/C（active 3/planned 4） | 16 平台手工投影 | **>**（策略：填表非重写；conformance > contract 约定） | test-adapters 58 断言；#39 |
...
| D14 | 平台广度 | 可移植内核 + 填表式扩展策略就绪 | 30 平台 | ~（策略就绪，铺量待 #40） | registry.yaml；#39 |
...
| D16 | loop-engineering 治理 | registry schema + enforce R1-R11 + L1→L3 分级放权 + token budget + circuit breaker（drift/graduation 进行中） | 两竞品都无 loop 安全/治理面 | 独有（~） | loops+budget 60 例；#35/#36，drift #37 |
| — | 无伪测试证据链 | 真 fs/真 HTTP/真 socket/真进程 e2e + TEST-REALITY 审计 + 诚实 skip | 用户硬要求，向 Trellis「真工具链」学习并制度化 | 独有 | docs/TEST-REALITY.md；C9/C10 |

## 汇总（截至 iteration-22）

- **> 更强**：D2/D4/D7/D8/D9/D10/D11/D12（8 维明确超越）
- **≥ 不弱**：D1/D3/D5/D6/D13（5 维持平或略强）
- **~ 进行中**：D14（平台铺量 #40）/ D16 drift·graduation（#37/#38）
- **独有护城河**：D15 golden-oracle · D16 loop 治理 · 无伪测试证据链

**收敛判定**：D14 铺量 + D16 drift/graduation 达标后，清单 D 全绿——届时"任何方面都 ≥ 且核心维度 >"成立。
