 ...
 * hash during decide -> evolve -> persist. */
export function digestAggregate(value: unknown): `sha256:${string}` {
  const normalized = value !== null && typeof value === 'object' && !Array.isArray(value)
    ? Object.fromEntries(Object.entries(value as Record<string, unknown>).filter(([key]) => key !== 'event_head_digest'))
    : value
  return `sha256:${sha256Hex(stable(normalized))}`
}
function freeze<T>(value: T): T { if (value !== null && typeof value === 'object' && !Object.isFrozen(value)) { for (const entry of Object.values(value as Record<string, unknown>)) freeze(entry); Object.freeze(value) } return value }
export function createAggregateV2(projectId: string, changeId: string, correlationId: string, updatedAt = '1970-01-01T00:00:00.000Z'): OrchestrationAggregateV2 { return freeze({ schema_version: 'board-snapshot/v2', record_id: `change:${changeId}`, project_id: projectId, change_id: changeId, revision: 0, correlation_id: correlationId, actor: { kind: 'system', id: 'kernel' }, created_at: updatedAt, status: 'draft', work_items: [], runs: [], results: [], validations: [], gates: [], leases: [], blockers: [], next_actions: ['accept-request', 'record-context'], updated_at: updatedAt }) }
...
export function evolveV2(aggregate: OrchestrationAggregateV2, event: BoardEventV2): OrchestrationAggregateV2 {
  ...
  let next: BoardSnapshotV2 = { ...aggregate, revision: event.revision, event_head_id: event.event_id, event_head_digest: event.after_digest, command_head_id: event.command_id, updated_at: event.issued_at }
  switch (command.type) {
    case 'accept-request': next = { ...next, request: command.request, status: 'draft', next_actions: ['record-context'] }; break
    case 'record-context': next = { ...next, context: command.context, status: 'assessing', next_actions: ['record-assessment'] }; break
    case 'record-assessment': next = { ...next, assessment: command.assessment, status: command.assessment.normalization === 'complete' ? 'planning' : command.assessment.normalization === 'needs-input' ? 'waiting-input' : 'blocked', next_actions: command.assessment.normalization === 'complete' ? ['freeze-pipeline'] : ['record-assessment'] }; break
    case 'freeze-pipeline': next = { ...next, pipeline: command.pipeline, status: 'planning', next_actions: ['freeze-work-graph'] }; break
    case 'freeze-work-graph': next = { ...next, graph: command.graph, status: 'planned', work_items: initialItems(aggregate, command.graph), next_actions: ['resolve-capabilities'] }; break
    case 'resolve-capabilities': next = { ...next, resolution: command.resolution, status: command.resolution.status === 'resolved' ? 'ready' : command.resolution.status === 'needs-input' ? 'waiting-input' : 'blocked', next_actions: command.resolution.status === 'resolved' ? ['start-change'] : ['resolve-capabilities'] }; break
    case 'start-change': next = { ...next, status: 'executing', next_actions: ['enqueue-work-item'] }; break
    case 'enqueue-work-item': { const item = find(next.work_items, 'work_item_id', command.work_item_id); if (item !== undefined) next = { ...next, work_items: replaceBy(next.work_items, 'work_item_id', { ...item, status: 'queued', revision: event.revision }), next_actions: ['claim-run'] }; break }
    case 'claim-run': { const item = find(next.work_items, 'work_item_id', command.run.work_item_id); const run = { ...command.run, status: 'claimed' as const, lease: command.lease, revision: event.revision }; next = { ...next, runs: [...next.runs.filter((entry) => entry.run_id !== run.run_id), run], leases: [...next.leases.filter((entry) => entry.lease_id !== command.lease.lease_id), command.lease], work_items: item === undefined ? next.work_items : replaceBy(next.work_items, 'work_item_id', { ...item, status: 'claimed', active_run_id: run.run_id, revision: event.revision }) }; break }
    ...
    case 'retry-work-item': { const item = find(next.work_items, 'work_item_id', command.work_item_id); const previous = item?.active_run_id === undefined ? next.runs.filter((run) => run.work_item_id === command.work_item_id).at(-1) : find(next.runs, 'run_id', item.active_run_id); if (item !== undefined) { const nextAttempt = (previous?.attempt ?? item.attempt_count) + 1; const newRun: SkillRunV2 | undefined = previous === undefined ? undefined : { ...previous, record_id: `run:${command.run_id}`, run_id: command.run_id, attempt_id: command.attempt_id, attempt: nextAttempt, prior_attempt_id: previous.attempt_id, status: 'queued', result_id: undefined, failure: undefined, lease: undefined, started_at: undefined, finished_at: undefined, input_manifest: undefined, revision: event.revision }; next = { ...next, ...(newRun === undefined ? {} : { runs: [...next.runs, newRun] }), work_items: replaceBy(next.work_items, 'work_item_id', { ...item, status: 'queued', attempt_count: nextAttempt, active_run_id: newRun?.run_id, blockers: [], revision: event.revision }), next_actions: ['claim-run'] } } break }
    ...
    case 'replan-change': next = { ...next, status: 'planning', pipeline: next.pipeline === undefined ? undefined : { ...next.pipeline, status: 'superseded', revision: event.revision }, graph: next.graph === undefined ? undefined : { ...next.graph, status: 'superseded', revision: event.revision }, resolution: undefined, next_actions: ['freeze-pipeline'] }; break
...
export function foldV2(events: readonly BoardEventV2[], initial: OrchestrationAggregateV2): OrchestrationAggregateV2 { return events.reduce(evolveV2, initial) }
