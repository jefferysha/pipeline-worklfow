...
export function evolveV2(aggregate: OrchestrationAggregateV2, event: BoardEventV2): OrchestrationAggregateV2 {
  ...
  switch (command.type) {
    ...
    case 'record-assessment': next = { ...next, assessment: command.assessment, status: command.assessment.normalization === 'complete' ? 'planning' : command.assessment.normalization === 'needs-input' ? 'waiting-input' : 'blocked', next_actions: command.assessment.normalization === 'complete' ? ['freeze-work-graph'] : ['record-assessment'] }; break
