  ...
  return (
    ...
    >
      ...
      <span className="col-start-2 row-start-1 flex min-w-0 flex-col sm:w-[240px] sm:flex-none">
        ...
        <span className="truncate font-mono text-[11px] leading-4 text-text-3" title={row.root}>
          {row.root}
        </span>
      </span>
...
export function ProjectsView({ snapshot, rulesByKey, onOpenProject }: ProjectsViewProps): JSX.Element {
  ...
  const rowId = (row: ProjectRow): string => duplicateBasenames.has(row.basename)
        ...
        <div className="flex flex-col gap-7">
          {needRows.length > 0 && (
            <div data-testid="section-need">
              ...
              <div className="flex flex-col gap-2.5">
                {needRows.map((row) => (
                  <ProjectRowButton key={row.root} row={row} rowId={rowId(row)} need onOpen={onOpenProject} t={t} />
                ))}
              </div>
            </div>
          )}
          ...
          {restRows.length > 0 && (
            <div data-testid="section-rest">
              ...
              <div className="flex flex-col gap-2.5">
                {restRows.map((row) => (
                  <ProjectRowButton key={row.root} row={row} rowId={rowId(row)} need={false} onOpen={onOpenProject} t={t} />
                ))}
              </div>
            </div>
          )}
            ...
            <div data-testid="section-unreachable" data-anim="pv-item">
              ...
              {unreachableOpen && (
                <div className="mt-2 flex flex-col gap-1">
                  {unreachable.map((row) => {
                    const projectRowId = rowId(row)
                    return (
                      <div
                        key={row.root}
                        data-testid={projectRowId}
                        data-ok="false"
                        aria-disabled="true"
                      ...
                      >
                        ...
                        <span className="flex min-w-0 flex-1 flex-col">
                          ...
                          <span className="truncate font-mono text-[11px] text-text-3" title={row.root}>{row.root}</span>
                        </span>
