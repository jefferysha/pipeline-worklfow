  ...
  return (
    <article className={operationCard}>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <select name="triage-source" className={operationInput} data-testid="ops-triage-source" value={props.source} onChange={(event) => props.onSourceChange(event.target.value as OperationTriageCardProps['source'])}>
          <option value="git-commits">git-commits</option><option value="loop-run-terminals">loop-run-terminals</option>
        </select>
        <input className={operationInput} data-testid="ops-triage-model" name="triage-model" autoComplete="off" value={props.model} onChange={(event) => props.onModelChange(event.target.value)} placeholder={t('operations.model_default')} />
      </div>
