      ...
      <div className={`grid gap-3 ${compact ? 'grid-cols-1' : 'lg:grid-cols-2'}`}>
        {shows('cadence') && <article className={`rounded-lg border border-border bg-fill/40 px-3.5 py-3 ${compact ? '' : 'lg:col-span-2'}`} data-testid="ops-cadence-status" data-enabled={cadence?.enabled === true}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="rounded-full border border-border bg-bg px-2 py-1 font-mono text-[10.5px] text-text-3">
          ...
          {cadence !== null && cadence.loops.length === 0 && <p className="mt-3 text-xs text-text-3">{t('operations.cadence_empty')}</p>}
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              {cadence.loops.map((item) => (
                ...
                >
                  ...
                  {item.error && <div className="mt-1 text-red-d">{item.error}</div>}
                </div>
              ))}
            </div>
          )}
