...
export function OrchestrationLaneHeader({
  ...
  return (
    <div className="relative flex flex-col items-start gap-[9px] border-b border-border px-3.5 pt-[13px] pb-3">
      {lane.running && <span className="pointer-events-none absolute inset-0 z-[1] overflow-hidden rounded-t-2xl" aria-hidden="true"><i className="absolute inset-y-0 left-0 w-[46px] bg-[linear-gradient(105deg,transparent_6%,color-mix(in_srgb,var(--green)_42%,transparent)_50%,transparent_94%)] opacity-0" data-anim="wb-gloss" data-testid={`wb-flow-gloss-${lane.id}`} /></span>}
      <div className="relative z-[2] flex w-full items-center gap-[9px]">
        {canDrag && <span className={GRIP_LANE} data-testid={`wb-lane-grip-${lane.id}`} draggable aria-hidden="true" title={t('workbench.board_drag_lane')}>⠿</span>}
        ...
        {canRemove && <><span className="min-w-2 flex-1" /><button type="button" className={LANE_RM} data-testid={`wb-lane-rm-${lane.id}`} aria-label={`${t('workbench.board_rm_lane')} · ${lane.name}`} title={`${t('workbench.board_rm_lane')} · ${lane.name}`} onClick={(event) => { event.stopPropagation(); onRemove(lane.id) }}>×</button></>}
      </div>
      {hasBadges && (
