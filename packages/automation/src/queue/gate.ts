 ...
 *
 * 老仓真相源：
 *   - hooks/pipeline-gate.sh:16-25 —— PIPELINE_AFK=1 沙箱放行三门（confirm/review/interaction）。
...
/** 沙箱镜像内注入的放行 env（runChange 侧 env={PIPELINE_AFK:"1"}）。 */
export const PIPELINE_AFK_ENV = 'PIPELINE_AFK'
/** 调度器执行路径的旁路 env（认领后翻 scheduled 前的防御逃生口）。 */
export const AUTOMATION_RUNNER_ENV = 'PIPELINE_AUTOMATION_RUNNER'

/**
 * 是否放行沙箱内三门。严格 `=== '1'`：host 端该变量恒未设（或非 1），三门行为逐字不变
 * （人在环不被削弱）。老仓 pipeline-gate.sh:23 `[ "${PIPELINE_AFK:-}" = "1" ]`。
 */
export function afkGateBypasses(env: Record<string, string | undefined>): boolean {
  return env[PIPELINE_AFK_ENV] === '1'
}
