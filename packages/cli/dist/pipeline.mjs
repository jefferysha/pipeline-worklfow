...
async function release(lockDir) {
  try {
    await rmdir(lockDir);
  } catch {
  }
...
async function detectBaseBranch(repoRoot) {
  try {
    const head = await readFile(path2.join(repoRoot, ".git", "HEAD"), "utf8");
...
var FsStateStore = class {
  async read(changeDir2) {
    return parsePipeline(await readFile(stateFilePath(changeDir2), "utf8"));
  }
  ...
  async init(opts) {
    ...
    await writeFile(stateFilePath(changeDir2), content, { encoding: "utf8", flag: "wx" });
...
async function writeProjectRegistry(registryPath, roots) {
  ...
  await writeFile2(tmp, `${JSON.stringify(roots, null, 2)}
`, "utf8");
  await rename2(tmp, registryPath);
}
...
async function resolveChangeDir(cwd, name2) {
  ...
  try {
    if ((await stat2(exact)).isDirectory())
      return exact;
  } catch {
  }
  try {
    ...
    if (entry.type === "directory") {
      if (await isDirAbs(path4.join(cwd, fp))) {
        ...
        for (const mf of await listMdFiles(cwd, fp)) {
          ...
          let content = "";
          try {
            content = await readFile2(absMf, "utf8");
          } catch {
            content = "";
          }
      ...
      if (await isFileAbs(path4.join(cwd, fp))) {
        let content = "";
        try {
          content = await readFile2(path4.join(cwd, fp), "utf8");
        } catch {
          content = "";
        }
...
import { mkdir as mkdir4, readFile as readFile3, readdir as readdir3, rm as rm2, rmdir as rmdir2, stat as stat4, unlink, writeFile as writeFile3 } from "node:fs/promises";
...
function createOwnedFs() {
  return {
    readText: async (abs) => {
      try {
        return await readFile3(abs, "utf8");
      } catch {
        return void 0;
      }
    },
    writeText: async (abs, content) => {
      ...
      await writeFile3(abs, content, "utf8");
    },
    exists: async (abs) => {
      try {
        await stat4(abs);
    ...
    isDir: async (abs) => {
      try {
        return (await stat4(abs)).isDirectory();
      } catch {
        return false;
      }
    },
...
function parseScalarBlock(lines, start, path5, section) {
  const map = /* @__PURE__ */ new Map();
  let i = start;
  while (i < lines.length) {
    const raw = lines[i];
    ...
    if (!entry) {
      throw new ManifestError(`${path5}:${i + 1} ${section} \u6761\u76EE\u987B\u4E3A 'key: value'\uFF0C\u5F97\u5230 '${lines[i]}'`);
    }
...
function scanSections(text, path5) {
  ...
  while (i < lines.length) {
    ...
    if (!top) {
      throw new ManifestError(`${path5}:${i + 1} \u65E0\u6CD5\u89E3\u6790\u7684\u9876\u5C42\u884C '${lines[i]}'\uFF08\u7A84\u89E3\u6790\u5B50\u96C6\u5916\uFF09`);
    }
    const key = top[1];
    ...
    if (key === "phases" || key === "review_phases") {
      ...
      if (rest !== "")
        throw new ManifestError(`${path5}:${i + 1} router_patterns \u5FC5\u987B\u662F\u5757\u5C0F\u8282`);
      const r = parseScalarBlock(lines, i + 1, path5, "router_patterns");
...
function loadManifest(path5) {
  const text = readFileSync3(path5, "utf8");
...
function writeSidecar(fs, path5, seq2) {
  const tmp = `${path5}.tmp.${fs.pid}.${Date.now()}`;
...
function initialOffset(fs, path5, mode) {
  if (mode.fromStart || mode.sinceSeq !== void 0)
    return 0;
  return fs.size(path5) ?? 0;
}
function readNewEvents(fs, path5, state) {
  const size = fs.size(path5);
  if (size === void 0) {
    state.byteOffset = 0;
    state.carry = "";
    return [];
  ...
  if (size <= state.byteOffset)
    return [];
  const chunk = fs.readSlice(path5, state.byteOffset, size - state.byteOffset);
  if (chunk === void 0)
    return [];
...
async function* tailEvents(fs, path5, opts = {}) {
  const pollMs = opts.pollMs ?? 200;
  const sleep3 = opts.sleep ?? defaultSleep;
  const now = opts.now ?? Date.now;
  const state = {
    byteOffset: initialOffset(fs, path5, opts),
    carry: ""
  };
  let yielded = 0;
  const started = now();
  for (; ; ) {
    if (opts.aborted?.())
      return;
    for (const ev of readNewEvents(fs, path5, state)) {
...
function readPid(fs, path5) {
  const raw = fs.readText(path5);
  if (raw === void 0)
    return void 0;
...
async function runInboxWatcher(a) {
  ...
  const path5 = eventsPath(env, channel, scope);
  const stream = tailEvents(tailFs, path5, {
    fromStart: cursor === 0,
...
import { mkdir as mkdir5, rename as rename3, writeFile as writeFile4 } from "node:fs/promises";
...
async function writeDraftMarks(path5, ids) {
  ...
  await writeFile4(tmp, `${JSON.stringify({ version: 1, ids }, null, 2)}
`, "utf8");
  await rename3(tmp, path5);
}
async function addDraftMark(path5, id) {
  const existing = readDraftMarks(path5);
  if (existing.includes(id))
    return;
  await writeDraftMarks(path5, [...existing, id]);
}
...
function evaluateStepGuards(state, step, _ctx) {
  const failures = [];
...
function serve(opts) {
  ...
  const server = createServer((req, res) => {
    ...
    async function handle(body) {
      const path5 = req.url ?? "/";
      const cleanPath = path5.split("?", 1)[0];
      ...
      let upstreamPath = path5;
      if (stripPrefix && cleanPath.startsWith(stripPrefix))
        upstreamPath = path5.slice(stripPrefix.length) || "/";
      const fwdHeaders = {};
...
function isBedrockEventstreamPath(path5) {
  const cleanPath = path5.split("?", 1)[0].replace(/\/+$/, "");
...
function isCaptureOnlyRequest(path5, body) {
  const cleanPath = path5.split("?", 1)[0];
  ...
  if (isPlainObject2(body) && isPlainObject2(body.request))
    return isCaptureOnlyRequest(path5, body.request);
...
// packages/tap/dist/forward-proxy.js
function buildRespBody(path5, raw) {
  if (isBedrockEventstreamPath(path5)) {
        ...
        upstream.on("end", () => {
          ...
          if (captureGate) {
            const skip = shouldSkipTraceRecord({
              upstreamUrl: upstreamBaseUrl + path5,
              path: path5,
            ...
            if (!skip) {
              try {
                store2.appendRecord(sessionId, buildRecord({
                  ...
                  status,
                  respHeaders: upstream.headers,
                  respBody: buildRespBody(path5, raw),
                  upstreamBaseUrl,
                  transport: "forward"
                }));
              } catch {
...
function writeAtomic(path5, data, mode) {
  const tmp = path5 + ".tmp";
...
function checkManifest(p) {
  ...
  return red(
    "asset:manifest",
    `manifest \u4E0D\u53EF\u7528: ${err}`,
    `\u68C0\u67E5 ${join20(p.pluginRoot, "templates", "manifest.yaml")} \u662F\u5426\u5B58\u5728\u4E14\u7B26\u5408\u7A84 YAML \u5B50\u96C6\uFF08\u89C1\u6587\u4EF6\u5934\u6CE8\u91CA\uFF09`
  );
}
function gateAssetProblems(p) {
  const problems = [];
  if (!p.fileExists(join20(p.pluginRoot, "hooks", "hooks.json"))) problems.push("hooks/hooks.json \u7F3A\u5931");
  const gate = join20(p.pluginRoot, "hooks", "gate.sh");
...
function checkHookAssets(p) {
  const missing = [];
  if (!p.fileExists(join20(p.pluginRoot, "hooks", "hooks.json"))) missing.push("hooks/hooks.json \u7F3A\u5931");
  for (const s of HOOK_SCRIPTS) {
    const abs = join20(p.pluginRoot, "hooks", s);
...
function checkStatusline(p) {
  ...
  return yellow(
    ...
    `\u5728 ~/.claude/settings.json \u52A0 "statusLine": {"type": "command", "command": "bash ${join20(p.pluginRoot, "hooks", "statusline.sh")}"}`
  );
}
function checkTap(p) {
...
async function checkChanges(deps) {
  ...
  for (const name2 of names) {
    try {
      await deps.store.read(join20(root, name2));
    } catch (e) {
...
async function checkVerifySkills(p) {
  ...
  return red(
    ...
    `bash ${join20(p.pluginRoot, "tools", "verify-skills.sh")} \u67E5\u770B\u9010\u6761\u4FEE\u590D\u6307\u5F15`
  );
}
...
async function cmdInbox(deps, opts) {
  ...
  const changesRoot2 = join21(deps.cwd, "openspec", "changes");
  for (const name2 of await deps.listChanges(changesRoot2)) {
    if (seen.has(name2)) continue;
    let fields;
    try {
      fields = (await deps.store.read(join21(changesRoot2, name2))).fields;
    } catch (e) {
...
async function scanReadyFromFs(changesDir, store2) {
  ...
  for (const name2 of activeNames) {
    const changeDir2 = join22(changesDir, name2);
    let state;
    try {
  ...
  let archiveEntries = [];
  try {
    const archived = await readdir4(join22(changesDir, "archive"), { withFileTypes: true });
...
function createAutomation(deps) {
  ...
  const changesDir = join24(deps.repoRoot, "openspec", "changes");
  const changeDir2 = (name2) => join24(changesDir, name2);
  return {
    config,
    async enqueue(name2) {
...
var worktreePathFor = (repoDir, branch) => join25(repoDir, ".sandcastle", "worktrees", branch.replace(/\//g, "-"));
var addWorktree = async (exec, repoDir, branch) => {
  await mkdir6(join25(repoDir, ".sandcastle", "worktrees"), { recursive: true });
...
var hasCancelMarker = async (worktreePath) => access(join25(worktreePath, CANCEL_MARKER_FILE)).then(() => true, () => false);
...
var resolveLockDir = async (exec, hostRepoDir) => {
  ...
  return join26(resolve7(hostRepoDir, out || ".git"), "sandcastle-mergeback.lock.d");
};
...
var createLifecyclePorts = (deps) => {
  ...
  return {
    ...
    async createSandbox({ env, worktreePath }) {
      const gitMounts = await resolveGitMounts(join27(worktreePath, ".git")).catch(() => []);
      const dotGit = join27(worktreePath, ".git");
    ...
    async runWork(sandboxExec, name2, signal, runner) {
      ...
      const changeDir2 = join27(hostRepoDir, "openspec", "changes", name2);
      const logPath = join27(changeDir2, ".sandcastle-run.log");
...
var createDockerRunChange = (opts) => {
  ...
  const changeDir2 = (name2) => join28(hostRepoDir, "openspec", "changes", name2);
...
function hostRmrf(host) {
  return host.rmrf ?? ((path5) => {
    if (!path5.startsWith(host.env.root)) return;
    try {
      rmSync2(path5, { recursive: true, force: true });
    } catch {
    }
  });
}
...
function cmdWait(deps, host, p) {
  ...
  const since = sinceRaw !== void 0 ? Number.parseInt(sinceRaw, 10) : void 0;
...
async function cmdGenRouterSh(deps, manifestPath2) {
  ...
  try {
    readFileSync15(manifestPath2, "utf8");
...
import { mkdirSync as mkdirSync6, readFileSync as readFileSync16, readdirSync as readdirSync4, writeFileSync as writeFileSync5 } from "node:fs";
import { dirname as dirname6, isAbsolute as isAbsolute3, join as join29 } from "node:path";
import { createInterface as createInterface2 } from "node:readline/promises";
function readTopLevelScalars(absPath) {
  let text;
  try {
    text = readFileSync16(absPath, "utf8");
  } catch {
    return null;
  }
  const out = {};
...
function sandboxPipelineYaml(repoRoot, name2, worktree) {
  ...
  if (worktree && worktree.trim() !== "") {
    ...
    base = isAbsolute3(w) ? w : join29(repoRoot, w);
  } else {
    base = join29(repoRoot, ".sandcastle", "worktrees", `sandcastle-pipeline-${name2}`);
  }
  return join29(base, "openspec", "changes", name2, ".pipeline.yaml");
}
var REAL_LOOPS_FS = {
  loadRegistry: (repoRoot) => loadRegistry(repoRoot),
  readProgress: (repoRoot) => {
    try {
      return readFileSync16(join29(repoRoot, ".superpowers", "loops", "progress.md"), "utf8");
    } catch {
      return null;
    }
  },
  listChanges: (repoRoot, changePrefix) => {
    try {
      return readdirSync4(join29(repoRoot, "openspec", "changes"), { withFileTypes: true }).filter((e) => e.isDirectory() && e.name !== "archive" && e.name.startsWith(changePrefix)).map((e) => e.name).sort();
    } catch {
      return [];
    }
  },
  readChangeFields: (repoRoot, name2) => readTopLevelScalars(join29(repoRoot, "openspec", "changes", name2, ".pipeline.yaml")),
...
var REAL_DRIFT_FS = {
  ...
  readLoopDoc: (repoRoot) => {
    try {
      return readFileSync16(join29(repoRoot, "LOOP.md"), "utf8");
    } catch {
      return null;
    }
  }
...
var REAL_GRADUATION_FS = {
  ...
  readRegistryText: (repoRoot) => {
    try {
      return readFileSync16(join29(repoRoot, ".pipeline", "loops.yaml"), "utf8");
    } catch {
      return null;
    }
  },
  writeRegistryText: (repoRoot, text) => writeFileSync5(join29(repoRoot, ".pipeline", "loops.yaml"), text, "utf8")
};
...
var REAL_INIT_ENV = {
  fs: {
    readText: (path5) => {
      try {
        return readFileSync16(path5, "utf8");
      } catch (e) {
        if (e.code === "ENOENT") return null;
        throw e;
      }
    },
    createExclusive: (path5, text) => {
      mkdirSync6(dirname6(path5), { recursive: true });
      writeFileSync5(path5, text, { flag: "wx" });
    },
    overwrite: (path5, text) => writeFileSync5(path5, text, "utf8")
  },
  addDraftMark: (path5, id) => addDraftMark(path5, id),
...
async function cmdInit2(deps, args, env = REAL_INIT_ENV) {
  ...
  const loopsPath = join29(deps.cwd, ".pipeline", "loops.yaml");
...
import { mkdir as mkdir9, readFile as readFile4, rm as rm3, stat as stat6, writeFile as writeFile6 } from "node:fs/promises";
...
var REAL_FS = {
  ...
  writeText: async (abs, content) => {
    await mkdir9(dirname7(abs), { recursive: true });
...
async function cmdScaffoldSpec(deps, args, fs) {
  ...
  const abs = (rel) => join30(deps.cwd, rel);
...
var REAL_FS2 = {
  loadPackages: async (cwd) => {
    let text;
    try {
      text = await readFile5(join31(cwd, PROJECT_CONFIG_FILE), "utf8");
    } catch {
      return null;
    }
    try {
  ...
  bindPointer: async (cwd, name2) => {
    await writeFile7(join31(cwd, ACTIVE_POINTER_FILE), `${name2}
`, "utf8");
  }
};
...
function makeGuardCtx(cwd) {
  ...
  return (name2) => ({
    ...
    readFile: (p) => {
      try {
        return readFileSync17(abs(p), "utf8");
      } catch {
        return void 0;
      }
    },
    ...
    changeArchived: (dep) => {
      try {
        return readdirSync5(abs("openspec/changes/archive"), { withFileTypes: true }).some((e) => e.isDirectory() && e.name.endsWith(`-${dep}`));
      } catch {
        return false;
      }
    },
